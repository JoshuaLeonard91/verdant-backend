//! Server custom stickers.

use sqlx::{PgPool, Postgres, Transaction};

pub const MAX_SERVER_CUSTOM_STICKERS: i64 = 100;
...
pub async fn by_id(pool: &PgPool, id: i64) -> Result<Option<StickerRow>, sqlx::Error> {
    sqlx::query_as::<_, StickerRow>("SELECT * FROM stickers WHERE id = $1")
        .bind(id)
        .fetch_optional(pool)
        .await
}
...
pub async fn list_for_server(
    ...
    sqlx::query_as::<_, StickerRow>(
        "SELECT * FROM stickers WHERE server_id = $1 ORDER BY id ASC LIMIT $2",
    )
    .bind(server_id)
    .bind(MAX_SERVER_CUSTOM_STICKERS)
    .fetch_all(pool)
    .await
}

pub async fn count_for_server(pool: &PgPool, server_id: i64) -> Result<i64, sqlx::Error> {
    sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM stickers WHERE server_id = $1")
        .bind(server_id)
        .fetch_one(pool)
        .await
}
...
pub async fn is_at_server_limit(pool: &PgPool, server_id: i64) -> Result<bool, sqlx::Error> {
    count_for_server(pool, server_id)
        .await
        .map(|count| count >= MAX_SERVER_CUSTOM_STICKERS)
}
...
pub async fn insert_if_below_server_limit(
    ...
    let count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM stickers WHERE server_id = $1")
        .bind(server_id)
        .fetch_one(&mut *tx)
        .await?;

    if count >= MAX_SERVER_CUSTOM_STICKERS {
        tx.commit().await?;
        return Ok(false);
    }
    ...
    sqlx::query(
        r#"
        INSERT INTO stickers (id, server_id, name, url, created_by, created_at_ms)
        VALUES ($1,$2,$3,$4,$5,$6)
        "#,
    )
    .bind(id)
...
pub async fn rename(pool: &PgPool, id: i64, name: &str) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE stickers SET name = $2 WHERE id = $1")
        .bind(id)
        .bind(name)
        .execute(pool)
        .await?;
...
> {
    let mut tx = pool.begin().await?;
    let asset_id = sqlx::query_scalar::<_, Option<i64>>(
        "DELETE FROM stickers WHERE id = $1 RETURNING asset_id",
    )
    .bind(id)
...
mod tests {
    ...
    #[test]
    fn server_custom_sticker_quota_is_bounded() {
        assert!(SOURCE.contains("pub const MAX_SERVER_CUSTOM_STICKERS: i64 = 100"));
    ...
    fn quota_insert_uses_server_scoped_advisory_lock() {
        ...
        assert!(source.contains("COUNT(*) FROM stickers WHERE server_id = $1"));
        assert!(source.contains("count >= MAX_SERVER_CUSTOM_STICKERS"));
        assert!(source.contains("INSERT INTO stickers"));
    }
    ...
    fn delete_returns_last_asset_cleanup_from_single_delete_statement() {
        ...
        assert!(source.contains("DELETE FROM stickers WHERE id = $1 RETURNING asset_id"));
        assert!(!source.contains("SELECT asset_id FROM stickers"));
