...
- Keep `S3_ENDPOINT=http://minio:9000` for the bundled MinIO service, or point `S3_ENDPOINT`, `S3_BUCKET`, `S3_ACCESS_KEY`, `S3_SECRET_KEY`, and optional `S3_REGION` at another S3-compatible provider. Single-VPS Garage installs generated by Verdant Operator use `S3_REGION=garage`.
